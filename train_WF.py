...
from datasets import VOCDetection, COCODetection, CocoDataset, get_augumentation, detection_collate, WFDetection
...
parser.add_argument('--dataset', default='WF', choices=['WF', 'VOC', 'COCO'],
                    type=str, help='WF, VOC or COCO')
parser.add_argument('--dataset_root', default='./widerface/train/',
                    help='Dataset root directory path [/root/data/VOCdevkit/, /root/data/coco/, ./widerface/train/]')
...
train_dataset = []
if(args.dataset == 'WF'):
    train_dataset = WFDetection(root=args.dataset_root,
...
def train():
    ...
    for epoch in range(args.num_epoch):
        ...
        for idx, (images, annotations) in enumerate(train_dataloader):
            ...
            loss = classification_loss + 20*regression_loss
        ...
        torch.save(
            state, './saved/weights/checkpoint_{}_{}_{}.pth'.format(args.dataset, args.network, epoch))
    state = {
        'arch': arch,
    ...
    torch.save(state, './saved/weights/Final_{}.pth'.format(args.network))
